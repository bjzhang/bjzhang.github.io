...
3.	文章提到把key和value分开对于crash后的一致性有调整，WiscKey怎么做的没看懂。
...
5.	value log很大，如何垃圾回收？
    1.	新数据永远从head写入。Wisckey GC时从tail扫描，如果是有用的value，会写回head；没用的删除。TODO：问题上这样会不会大量多余的写？
    2.  WiscKey是每次读取4MB的数据。所以如果value比较小，例如1k，会用更多时间在LSM tree中查找。
...
1.	Crash consistency.
    1.  Wisckey对定期更新head pointer。最长恢复时间是正要更新head pointer之前crash。多久更新一次指针是可以配置的。
1.	Space Amplification
    1.  read amplication, write amplication, space amplication三个没法同时变小。
1.	Cpu Usage
    1.	BadgerIO和levelDB cpu都不是瓶颈，为什么TiDB cpu是瓶颈？
    2.  RocksDB做了对多核更好的concurrency.
